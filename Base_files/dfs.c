 ...
 * Compile with:
 *
 *     gcc -Wall dfs.c `pkg-config fuse3 --cflags --libs` -o dfs
 *
 * ## Source code ##
 * \include dfs.c
 */
...
static void show_help(const char *progname)
{
	...
	printf("File-system specific options:\n"
	       "    --name=<s>          Name of the \"dfs\" file\n"
	       "                        (default: \"dfs\")\n"
	       "    --contents=<s>      Contents \"dfs\" file\n"
	       "                        (default \"Nothing !\\n\")\n"
	       "\n");
}
...
int main(int argc, char *argv[])
{
	...
	char *file_conts = " \n \n \n \t \t \t Welcome to the world of DFSSSSSS .!!!!!!! \n \n \t \t \t We are just getting started with it. So hold tight for future work. \n \n  \t \t \t PEACE OUT /\../";

	/* Set defaults -- we have to use strdup so that
	   fuse_opt_parse can free the defaults if other
	   values are specified */

	options.filename = strdup("dfs_file");
	options.contents = strdup(file_conts);
