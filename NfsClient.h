...
class NfsClient {
 public:
  NfsClient(std::shared_ptr<Channel> channel)
	...
	std::cout<<"Entered file read"<<endl;
        ClientContext clientContext;
        ReadRequest rr;
	cout<<"Path is "<<path<<endl;
        rr.set_path(path);
	cout<<"Path post is "<<rr.path()<<endl;
        rr.set_size(size);
        rr.set_offset(offset);

        ReadResult rres;

	cout<<"Path post post is "<<rr.path()<<endl;
        Status status = stub_->nfsfuse_read(&clientContext, rr, &rres);
        if(rres.err() == 0){
	    std::cout<<"Received buffer is "<<rres.buffer().c_str()<<endl;
            strcpy(buf, rres.buffer().c_str());
	    std::cout<<"Received buffer is post copy "<<buf<<endl;
